...
import {JENotification} from "../libs/util.toast";
...
instance.interceptors.request.use(
    (config) => {
        ...
        console.log("TOKEN...",dbUtils.get('token'))
        config.headers.Authorization = `Bearer ${dbUtils.get('token')}`
    ...
    if (error.response) {
        ...
        switch (status) {
            case 400:
                ...
                JENotification.error(error.response.data.message || '参数校验失败')
        ...
        JENotification.error('请求未收到响应')
        // 在这里可以进行错误处理逻辑，例如弹出错误提示、记录错误日志等
    } else {
        // 请求配置出错
        console.error('请求配置出错:', error.message);
        JENotification.error('请求配置出错')
        // 在这里可以进行错误处理逻辑，例如弹出错误提示、记录错误日志等
    }
}
